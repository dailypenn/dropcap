...
const {
  client_email,
  private_key
} = require('./dropcap-service-credentials.json')
const { encodeHTML, getPagePath, combineAndStripURLs } = require('./util')
      ...
      ga.get(
        {
          ...
          filters: getPagePath(VIEWS[viewName].blogSlug)
        },
        (err, response) => {
          if (err) return reject(err)
          if (!response.rows)
            return reject(`Empty GA API response for ${VIEWS[viewName].name}`)
...
const urlDataAsJSON = (urlList, viewName) => {
  return new Promise((resolve, reject) => {
    ...
    for (const item of urlList) {
      ...
      mergeOGData(canonicalURL, item).then(
        data => {
          result.push(data)
...
app.get('/:view', (req, res) => {
  ...
  if (!VIEWS[view])
    return res.status(404).send({ error: `Unknown view ${view}` })
